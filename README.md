...
Actions is where you describe the action type and optional payload. Actions are then dispatched to the reducers so it updates the state. Reducers update the state in the store. And when I say update, it actually replace it because the store is immutable.

- [X] What is the difference between Application state and Component state? When would be a good time to use one over the other?

You should use Redux and Application State whenever you want to pass state all around your application because when you use component state, you can only pass state down to your children through props.

- [X] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Redux-Thunk enables us to do async operations which means async api calls for action creators.
...
- [X] **cd** into `smurfs` and run `yarn install` to retrieve the client side dependencies.
